...
a = SNB_data %>%
  ...
  left_join(weather_library) %>%
  select(sev, 
         "fa1.21_14.TR.19T22nR.G0.2.dawn.sum_25", 
         "fa1.-6_-12.TRH.13T16nRH.G80.dusk.sum_30",
         "fa2.12_-1.T.22T25.dusk.sum_30",
         "fa3.56_48.T.3T7.dusk.sum_20") %>%
  pivot_longer(cols = !sev, names_to = "predictor")

# Define the order of the predictors
predictor_order <- c("fa1.21_14.TR.19T22nR.G0.2.dawn.sum_25", 
                     "fa1.-6_-12.TRH.13T16nRH.G80.dusk.sum_30",
                     "fa2.12_-1.T.22T25.dusk.sum_30",
                     "fa3.56_48.T.3T7.dusk.sum_20")
...
wrap_plots(plot_list, ncol = 2) + plot_annotation(tag_levels = 'A') + plot_layout(axis_titles =  "collect") & theme(text = element_text(size = 12),
                                                                                                                    ...
                                                                                                                    plot.margin = margin(15, 15, 15, 15),  # Increases margin around the plo                                                                                                          axis.title.x = element_text(margin = margin(t = 10)),  # Adds space between x-axis title and the plot
